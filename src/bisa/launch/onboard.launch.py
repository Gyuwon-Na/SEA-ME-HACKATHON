...
def default_model_path():
    ...
    The on-car launch runs YOLO on the A72 CPU, where torch eager inference of
    best.pt is slow (~160 ms/frame standalone). The NCNN export is ~2.4x faster
    (~67 ms) at the same imgsz. The PC launches keep using best.pt on the GPU.
    Note: this NCNN model is exported at imgsz=320 (fixed) — keep imgsz:=320.
    """
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        # --- camera (MJPG passthrough → CompressedImage 발행) ---------------
        # MJPG 패스스루: 카메라 하드웨어 JPEG을 그대로 발행 (encode CPU 0).
        # raw 전환은 오히려 FPS 저하(3.4→2.3): cv2.imdecode는 GIL 해제(nogil C)라
        # 병목 아니었고, raw 921KB DDS 전달이 더 무거움 → compressed 유지.
        Node(
            package="camera",
            ...
            parameters=[{
                ...
                "publish_hz": 24.0,
                "debug_log": False,
                # V4L2 컨트롤 고정: focus_auto/exposure_auto/white_balance_auto OFF
                # exposure_auto_priority=0 으로 FPS 드롭 방지
        ...
        Node(
            ...
            parameters=[{
                ...
                "debug_log": False,
                # --- 보드 검출기 설정 (Vulkan GPU 오프로드) ---
                # PowerVR GT9524 Vulkan: per-frame은 CPU보다 느리나(9.7 vs 18 FPS
                # 격리) 컨볼루션을 GPU로 오프로드 → A72 CPU 점유 370%→18%
                # (코어 3.5개 확보). CPU가 포화된 온보드에선 이 오프로드가 핵심:
                # 카메라/lane/control 루프가 코어를 되찾아 실부하 팽창이 사라짐.
                "detector.device": "vulkan:0",
                "detector.imgsz": imgsz,
                # 2단 파이프라인(더블버퍼)으로 CLAHE를 GPU 뒤에 숨겨 ~9 FPS 목표.
                # 게이트가 향상분을 자르지 않게 12Hz로 상향(실제는 연산 상한에 수렴).
                "detector.inference_hz": 12.0,
                # sign_vote 윈도우: 실제 ~4-9 FPS 대응 4/6프레임 ≈ 0.7-1.5초 판정
                "detector.sign_vote_k": 4,
                "detector.sign_vote_n": 6,
            }],
        ),
    ])
